...
function run_game(orig_code)
	...
	scores = []
	while true
		i, x, rel = intcode!(code, input, i, rel)
		if x === nothing
			break
		elseif x == -1
			i, y, rel = intcode!(code, input, i, rel)
			i, score, rel = intcode!(code, input, i, rel)
			push!(scores, score)
		else
			i, y, rel = intcode!(code, input, i, rel)
			i, tile, rel = intcode!(code, input, i, rel)
			if tile == 4
				x_ball = x
				#@show x_ball, y
				#@show x_paddle
